...
We subject the output of the \texttt{RAND} algorithms described in section ~\ref{sec:algs} to the statistical tests described in section ~\ref{sec:stattests}. These are our results in the three different settings used in our experiments. 

We used a baudrate of 115200 bps (highest available for the Arduino over the FTDI RS232-to-USB connection) for these experiments for maximum bandwidth. The maximum reading rate of \texttt{analogRead} is about 10000 times per second\cite{ardref}. When we read\footnote{Done with \texttt{poll.py} in the Ardrand codebase} iteratively from \texttt{analogRead} over USB,  \texttt{pyserial} will raise either an \texttt{OSError} or \texttt{SerialException}. This happens approximately once every 500 times and reading the \texttt{pyserial} source code\footnote{See \url{http://www.java2s.com/Open-Source/Python/Development/pySerial/pyserial-2.5-rc2/serial/serialposix.py.htm}} we find a comment stating that ``disconnected devices, at least on Linux, show the behavior that they are always ready to read immediately but reading returns nothing''. We therefore conclude that these exceptions are the result of \texttt{analogRead} blocking for moment. 

These are the results of our \texttt{RAND} algorithms in all three settings, subjected to the FIPS boundaries. Each bitsequence tested is 20000 bits long and each recorded result is the average of three consecutive runs. 

\subsubsection{Results in setting 0}

  \begin{table}[H]
    \begin{center}
      \begin{tabular}{| l | l | l | l |}
        \hline
        Statistical test & Result & Accepted? & Bandwidth\\
        \hline
        \hline
        \textbf{Monobit} & $X_1 = 903.847$ & \textcolor{red}{\textbf{Rejected}} & $9.964 < X_1 < 10.346$  \\
        \textbf{Poker} & $X_3 = 3211.45$ & \textcolor{red}{\textbf{Rejected}} & $1.03 < X_3 < 57.4$ \\
        \textbf{Runs} & $X_4 = 2812.81$ & \textcolor{red}{\textbf{Rejected}} & Lengths of runs used \\
        \textbf{Long runs} & & \textcolor{green}{\textbf{Accepted}} & No run longer than 34 \\
        \hline
      \end{tabular}
    \end{center}
    \caption{\texttt{Leastsign-RAND} in setting 0}
    \label{tab:leastsign0}
  \end{table}



\subsubsection{Results in setting 1}

\subsubsection{Results in setting 2}
